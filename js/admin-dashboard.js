...
async function loadOverviewStats() {
    try {
        ...
        // 2. Comprehensive Totals (Revenue, Profit, Liability)
...
function applyOrderFilters() {
    ...
    const sDate = document.getElementById("filterDateFrom").value;
...
async function massUpdateSelected() {
    ...
    if(!confirm(`Are you sure you want to MASS FULFILL ${checked.length} orders?`)) return;
...
async function loadPricingConfig() {
    try {
        ...
        const { data: settings } = await supabase
            .from('app_settings')
            .select('key, value')
            .in('key', ['manual_transfer_enabled', 'manual_momo_number', 'manual_momo_name', 'maintenance_mode']);
...
async function loadManualFunding() {
    ...
    tbody.innerHTML = `<tr><td colspan="6" class="state-msg">Fetching manual transfers...</td></tr>`;

    // Get selected filters
    const statusFilter = document.getElementById("manualStatusFilter")?.value || "";
    const emailSearch = document.getElementById("manualSearchEmail")?.value.toLowerCase() || "";
    const refSearch = document.getElementById("manualSearchRef")?.value.toLowerCase() || "";

    // Fetch manual transfers
    let query = supabase
        .from('transactions')
    ...
    if (error || !data || data.length === 0) {
        tbody.innerHTML = `<tr><td colspan="6" class="state-msg">No manual transfers found.</td></tr>`;
        return;
    }

    // Apply client-side filtering for search
    let filteredData = data.filter(t => {
        const emailMatch = !emailSearch || (t.users?.email || '').toLowerCase().includes(emailSearch);
        const refMatch = !refSearch || (t.reference || t.id.substring(0, 8)).toLowerCase().includes(refSearch);
        return emailMatch && refMatch;
    });

    if (filteredData.length === 0) {
        tbody.innerHTML = `<tr><td colspan="6" class="state-msg">No matching transfers found.</td></tr>`;
        return;
    }
    ...
    filteredData.forEach(t => {
        ...
        const statusBadge = t.status === 'pending' 
            ? '<span style="background:#fbbf24; color:#78350f; padding:4px 8px; border-radius:4px; font-size:11px; font-weight:600;">PENDING</span>'
            : t.status === 'completed'
            ? '<span style="background:#86efac; color:#166534; padding:4px 8px; border-radius:4px; font-size:11px; font-weight:600;">✓ COMPLETED</span>'
            : '<span style="background:#fca5a5; color:#7f1d1d; padding:4px 8px; border-radius:4px; font-size:11px; font-weight:600;">✕ REJECTED</span>';
        
        html += `
            <tr>
                <td style="font-family:monospace; color:#64748b;">${t.reference || t.id.substring(0,8)}...</td>
                ...
                <td>
                    ${t.status === 'pending' ? `
                        <button class="action-btn btn-approve" onclick="approveFunding('${t.id}', '${t.user_id}', ${t.amount})">Approve</button>
...
async function approveFunding(txId, userId, amount) {
    ...
    try {
        ...
        const { data: u, error: userError } = await supabase.from('users').select('id, wallet_balance, phone, email').eq('id', userId).single();
        if (userError || !u) {
            console.error("User fetch error:", userError);
            alert("User not found or unable to fetch user details.");
            return;
        }

        // 2. Calculate new balance - ensure proper numeric conversion
        const currentBalance = parseFloat(u.wallet_balance || 0);
        const amountToAdd = parseFloat(amount || 0);
        const newBalance = currentBalance + amountToAdd;
        
        console.log(`Updating user ${userId}: Current=${currentBalance}, Adding=${amountToAdd}, New=${newBalance}`);
        
        // 3. Update user's wallet balance with proper NUMERIC type
        const { error: walletError, data: updateResult } = await supabase
            .from('users')
        ...
        // 4. Update transaction status to completed with description
        const { error: txError } = await supabase.from('transactions').update({ 
            status: 'completed',
            description: `Admin approved manual funding. Previous balance: ₵${currentBalance.toFixed(2)}, New balance: ₵${newBalance.toFixed(2)}`
        }).eq('id', txId);
        ...
        if (verifyError) {
            ...
            console.log("Verified wallet balance:", verifiedUser?.wallet_balance);
        }

        alert(`✅ Wallet Updated Successfully!\n\nUser: ${u.email}\nAmount Added: ₵${amountToAdd.toFixed(2)}\nPrevious Balance: ₵${currentBalance.toFixed(2)}\nNew Balance: ₵${newBalance.toFixed(2)}\n\nPlease refresh user page to see changes.`);
        loadManualFunding();
    } catch (error) {
...
async function loadSupportTickets() {
    ...
    tbody.innerHTML = `<tr><td colspan="7" class="state-msg">Fetching support tickets...</td></tr>`;

    // Get selected filters
    const statusFilter = document.getElementById("ticketStatusFilter")?.value || "";
    const ticketIdSearch = document.getElementById("ticketSearchId")?.value.toLowerCase() || "";
    const phoneSearch = document.getElementById("ticketSearchPhone")?.value.toLowerCase() || "";
    const issueSearch = document.getElementById("ticketSearchIssue")?.value.toLowerCase() || "";
    ...
    if (error || !data || data.length === 0) {
        tbody.innerHTML = `<tr><td colspan="7" class="state-msg">No support tickets found.</td></tr>`;
        return;
    }

    // Apply client-side filtering for search
    let filteredData = data.filter(t => {
        const ticketIdMatch = !ticketIdSearch || t.id.toLowerCase().includes(ticketIdSearch);
        const phoneMatch = !phoneSearch || (t.phone || '').toLowerCase().includes(phoneSearch);
        const issueMatch = !issueSearch || (t.issue || '').toLowerCase().includes(issueSearch);
        return ticketIdMatch && phoneMatch && issueMatch;
    });

    if (filteredData.length === 0) {
        tbody.innerHTML = `<tr><td colspan="7" class="state-msg">No matching tickets found.</td></tr>`;
        return;
    }
    ...
    filteredData.forEach(t => {
        ...
        const statusBadge = t.status === 'checking'
            ? '<span style="background:#bfdbfe; color:#1e40af; padding:4px 8px; border-radius:4px; font-size:11px; font-weight:600;">REVIEWING</span>'
            : t.status === 'approved'
            ? '<span style="background:#86efac; color:#166534; padding:4px 8px; border-radius:4px; font-size:11px; font-weight:600;">✓ APPROVED</span>'
            : '<span style="background:#fca5a5; color:#7f1d1d; padding:4px 8px; border-radius:4px; font-size:11px; font-weight:600;">✕ INVALID</span>';
        ...
        html += `
            <tr>
                ...
                <td>
                    <button class="action-btn btn-view" onclick="openImageModal('${imgUrl}')">View</button>
                </td>
                <td>${statusBadge}</td>
                <td>
                    ${t.status === 'checking' ? `
                        <button class="action-btn btn-approve" onclick="resolveTicket('${t.id}', 'approved')">Approve</button>
                        <button class="action-btn btn-reject" onclick="resolveTicket('${t.id}', 'false')">Invalid</button>
                    ` : '<span style="color:#64748b;">—</span>'}
                </td>
            </tr>
        `;
...
async function generateProfitReport() {
    const targetDate = document.getElementById('profitDateFrom').value;
